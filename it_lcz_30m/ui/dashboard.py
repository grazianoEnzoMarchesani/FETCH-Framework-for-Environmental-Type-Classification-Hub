...
class ITLCZDashboard(QDockWidget):
    def __init__(self, iface, parent=None):
        ...
        # 4. Grid Definition
        self.grid_group = QGroupBox("4. Definizione Griglia LCZ")
        self.grid_layout = QVBoxLayout(self.grid_group)
        
        # Radio buttons for grid mode selection
        ...
        self.cell_size_layout.addWidget(QLabel("Dimensione cella:"))
        self.cell_size_combo = QComboBox()
        self.cell_size_combo.addItems(["30m", "50m", "100m"])
        self.cell_size_combo.setCurrentIndex(2)  # Default 100m
        self.cell_size_layout.addWidget(self.cell_size_combo)
        self.grid_layout.addLayout(self.cell_size_layout)
        
        # Info label for estimated cells
        self.grid_info_label = QLabel("(Seleziona AOI per calcolare celle)")
        self.grid_info_label.setStyleSheet("font-size: 10px; color: #7f8c8d; font-style: italic;")
        self.grid_layout.addWidget(self.grid_info_label)
        
        self.grid_layout.addSpacing(5)
        
        # Use existing layer option
        self.grid_layer_radio = QRadioButton("Usa layer vettoriale esistente")
        ...
        # Toggle logic for grid mode
        self.grid_auto_radio.toggled.connect(self.toggle_grid_mode)
        
        self.grid_layout.addSpacing(10)
        
        # Generate grid button
        self.btn_grid = QPushButton("Genera/Applica Griglia")
        self.btn_grid.setStyleSheet("background-color: #e67e22; color: white; font-weight: bold; padding: 5px;")
    ...
    def run_grid_creation(self):
        ...
        if self.grid_auto_radio.isChecked():
            ...
            self.status_label.setText(f"Generazione griglia {cell_size}m...")
            QApplication.processEvents()
            
            success, message, output_path = self.data_manager.create_lcz_grid(
                extent, crs, cell_size=cell_size, log_callback=log_callback
            )
        else:
            # Use existing layer
            ...
            self.status_label.setText("Applicazione layer esistente come griglia...")
            QApplication.processEvents()
            
            success, message, output_path = self.data_manager.use_existing_grid(
                existing_layer, extent, crs, log_callback=log_callback
            )
        
        if success and output_path:
            # Load grid into project
            ...
            if loaded_layer:
                ...
                self.iface.messageBar().pushMessage(
                    "IT-LCZ", f"Griglia LCZ creata con successo! {loaded_layer.featureCount()} celle.", level=3
                )
            else:
                self.progress.setMaximum(100)
                self.progress.setValue(50)
                self.status_label.setText("Griglia creata ma non caricata nel progetto.")
        else:
            self.progress.setMaximum(100)
            self.progress.setValue(0)
