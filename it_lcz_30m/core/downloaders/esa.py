...
class ESADownloader(BaseDownloader):
    ...
    def calculate_tiles(self, extent, crs_auth_id):
        ...
        try:
            ...
            self.log(f"Error transforming extent to WGS84 for ESA: {e}", Qgis.Critical)
            return []
    ...
    def fetch_worldcover(self, extent, crs_auth_id):
        self.log("Avvio acquisizione ESA WorldCover (Land Use)...")
        ...
        for tile_name in tile_names:
            ...
            if success:
                self.log(f"ESA WorldCover tile {tile_name} scaricata.")
            else:
                self.log(f"Errore download ESA tile {tile_name}: {msg}", Qgis.Warning)
